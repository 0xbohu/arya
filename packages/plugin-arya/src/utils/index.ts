...
export const getTwitchCredentials = (runtime: IAgentRuntime) => {
    return {
        TWITCH_CLIENT_ID: runtime.getSetting("TWITCH_CLIENT_ID"),
        TWITCH_ACCESS_TOKEN: runtime.getSetting("TWITCH_ACCESS_TOKEN"),
    };
};

export const getYoutubeCredentials = (runtime: IAgentRuntime) => {
    return {
        YOUTUBE_API_KEY: runtime.getSetting("YOUTUBE_API_KEY"),
    };
};

export const getMediaUserStarknetIDAddress = (text: string) => {
    var sa = "";
    var sid = "";
    if (text.indexOf("[sa]") > -1 && text.indexOf("[/sa]") > -1) {
        var saStartPos = text.indexOf("[sa]");
        var saEndPos = text.indexOf("[/sa]");
        sa = text.substring(saStartPos + "[sa]".length, saEndPos).trim();
    }

    if (text.indexOf("[sid]") > -1 && text.indexOf("[/sid]") > -1) {
        var sidStartPos = text.indexOf("[sid]");
        var sidEndPos = text.indexOf("[/sid]");
        sid = text.substring(sidStartPos + "[sid]".length, sidEndPos).trim();
    }
    return {
        STARK_ID: sid,
        STARK_ADDRESS: sa,
